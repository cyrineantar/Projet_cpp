...
#include "ui_espace_fournisseur.h"
...
Espace_fournisseur::Espace_fournisseur(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Espace_fournisseur)
{
    ui->setupUi(this);
...
//cb matricule fournisseur
QSqlQueryModel* model=new QSqlQueryModel();
model->setQuery("SELECT Matricule FROM fournisseur");
ui->cb_matF->setModel(model);
ui->cb_matF_modif->setModel(model);
}

Espace_fournisseur::~Espace_fournisseur()
{
    delete ui;
}

void Espace_fournisseur::on_pb_ajouter_clicked()
{     son->play();
...
void Espace_fournisseur::on_pb_supprimer_clicked()
{    son->play();
...
void Espace_fournisseur::on_pb_ajoutercommande_clicked()
{    son->play();
...
QString fournisseur=ui->cb_matF->currentText();
...
void Espace_fournisseur::on_tabfournisseur_activated()
{
...
void Espace_fournisseur::on_pb_modifier_2_clicked()
{       son->play();
...
void Espace_fournisseur::on_le_recherche_textChanged(const QString &arg1)
{
    fournisseur F;
       ui->tabfournisseur->setModel(F.rechercher(ui->cb_recherche->currentText(),arg1));
}

void Espace_fournisseur::on_tabfournisseur_clicked(const QModelIndex &index)
{
    fournisseurSelect=ui->tabfournisseur->model()->data(index.sibling(index.row(),0)).toString();

}

void Espace_fournisseur::on_pb_supprimer_2_clicked()
{        son->play();
    achat a;
    achat a1;
    QMessageBox msgBox;
...
void Espace_fournisseur::on_le_recherche_2_textChanged(const QString &arg1)
{
    achat a;
       ui->tabachat->setModel(a.rechercher(ui->comboBox->currentText(),arg1));
}

void Espace_fournisseur::on_tabachat_clicked(const QModelIndex &index)
{
    achatselect=ui->tabachat->model()->data(index.sibling(index.row(),0)).toInt();

}

void Espace_fournisseur::on_pb_modifier_5_clicked()
{     son->play();
    ...
    QString prix, description, fournisseur,date_achat, produit;
        ...
        fournisseur=ui->cb_matF_modif->currentText();
...
void Espace_fournisseur::on_tabachat_activated()
{
    QSqlQuery query;
    ...
    if (query.exec())
    {
        ...
        while(query.next())
        {
            ...
            ui->cb_matF_modif->setCurrentIndex(ui->cb_matF_modif->findText(query.value(4).toString()));
...
void Espace_fournisseur::on_tri_clicked()
{     son->play();
...
void Espace_fournisseur::on_pushButton_clicked()
{     son->play();
    QPrinter printer;
...
void Espace_fournisseur::on_PDF_clicked()
{     son->play();
    QString strStream;
...
void Espace_fournisseur::on_tri_fournisseur_clicked()
{     son->play();
...
void Espace_fournisseur::on_imprimer_fournisseur_clicked()
{     son->play();
    QPrinter printer;
                   QPrintDialog *printDialog = new QPrintDialog(&printer, this);
                   printDialog->setWindowTitle("Imprimer Document");
...
void Espace_fournisseur::on_Excel_clicked()
{
    QTableView *table;
